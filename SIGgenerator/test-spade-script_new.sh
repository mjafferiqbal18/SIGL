...
start_capture() {
    tshark -i any -f "port 53" -w /home/vagrant/sigl_dns_logs/dns_capture.pcap &
    # Save the process ID of tshark to a file for later use
    echo $! > /tmp/tshark_pid
...
stop_capture() {
    ...
    if [ -f "/tmp/tshark_pid" ]; then
        # Read the process ID from the file
        tshark_pid=$(cat /tmp/tshark_pid)
        ...
        echo "Packet capture process not found."
    fi
}
...
main_function(){
  ...
  dir_path="/home/vagrant/sigl_dns_logs/"
  ...
  do
    ...
    #mv /tmp/$name-$i.log  /home/vagrant/sigl_logs/
  done
